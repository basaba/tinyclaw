...
**TUI:** Press `s` on the workflow list screen to open the gallery. Use `/` to search, arrow keys to navigate, and `Enter` to install a sample.

**Web UI:** Click the **📦 Samples** button in the toolbar (or **📦 Browse Sample Gallery** if no workflows are configured).
